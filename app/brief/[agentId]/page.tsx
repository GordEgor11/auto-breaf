...
export default function AgentBriefPage() {
  ...
  useEffect(() => {
    const raw = localStorage.getItem(`brief_draft_v1_${agentId}`);
    if (!raw) return;
    try {
      const parsed = JSON.parse(raw) as Partial<LeadFormState>;
      setForm((prev) => ({ ...prev, ...parsed }));
    } catch {
      localStorage.removeItem(`brief_draft_v1_${agentId}`);
    }
  }, [agentId]);

  useEffect(() => {
    localStorage.setItem(`brief_draft_v1_${agentId}`, JSON.stringify(form));
  }, [form, agentId]);
  ...
  useEffect(() => {
    ...
    void fetch("/api/lead-events", {
      ...
      body: JSON.stringify({ event_type: "form_view", agent_id: agentId }),
    });
  }, [agentId]);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      void fetch("/api/lead-events", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ event_type: "form_submit", agent_id: agentId }),
      });
      ...
      void fetch("/api/lead-events", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ event_type: "form_success", agent_id: agentId }),
      });
    } catch (error) {
      const errorMessage =
    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-900">
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-16">
        ...
        {status === "success" && leadId ? (
          <section className="flex flex-col gap-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-8 shadow-sm">
            ...
            <div className="flex gap-3">
              ...
              <button
                className="inline-flex h-11 w-fit items-center justify-center rounded-full border border-zinc-300 px-6 text-sm font-semibold text-zinc-900 transition hover:border-zinc-400"
                type="button"
        ...
        <div className="flex items-center justify-between text-sm text-zinc-600">
          <p className="text-xs text-zinc-500">
            Персональная ссылка: <code className="bg-zinc-100 px-2 py-1 rounded">{typeof window !== 'undefined' ? window.location.href : ''}</code>
          </p>
          <button
            onClick={handleCopyLink}
